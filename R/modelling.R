...
get_precision_single <- function(response, target = 0) {
  ...
  x <- logspace(-2, 2, 100)
  p0 <- trapz(x, n / (sqrt(response) * exp(response + (n * exp(-response)))))
